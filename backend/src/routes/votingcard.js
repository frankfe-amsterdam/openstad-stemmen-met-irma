...
const irmaRequest = require('../services/selectIrmaRequest');
...
router.get('/:id/disclose/finish', (req, res) => {
    ...
    if (!data.discloseToken) return res.status(403).json({ err: 'no disclosure started yet for this session' });
    ...
    return irmaBackend
        ...
        .then(result => {
            ...
            let getValue = (result, id) =>
                result.disclosed[0].filter(attr => attr.id == id && attr.status == 'PRESENT')[0].rawvalue;

            let ids = irmaRequest.disclose.flat(2);
...
router.get('/:id/issue/start', (req, res) => {
    ...
    try {
        ...
        let row = req.db.prepare('SELECT name, start, end FROM elections WHERE id = ?').get(req.params.id);
                ...
                credentials: [
                    {
                        ...
                        attributes: {
                            election: row.name,
                            voteURL: 'https://irmapilot.cms.special-branch.openstad.amsterdam/',
                            start: row.start,
                            end: row.end
                        }
                    }
        ...
        irmaBackend
            ...
            .then(result => {
                ...
                req.db.transaction(() => {
                    ...
                    console.log(`id: ${req.params.id}, identity: ${JSON.stringify(data.disclosed)}`);

                    req.db
                        .prepare(`UPDATE elections SET participants = participants + 1 WHERE id = ?`)
                        .run(req.params.id);
                })();
